  ...
  for (let i = 0; i <= attempts; i++) {
    try { return await fn(); }
      ...
      if (i < attempts) {
        ...
        console.warn(`[emailService] attempt ${i+1} failed, retrying in ${delay}ms:`, err && err.message ? err.message : err);
        await sleep(delay);
      }
    }
  }
...
async function sendCriticalVulnEmail({ to, userName, scan_task_id, website_url, vulnSummary }) {
  if (!emailsEnabled()) {
    console.log('[emailService] EMAIL_ENABLED != true — skipping sendCriticalVulnEmail', { to, scan_task_id });
  ...
  if (process.env.SENDGRID_API_KEY) {
    try {
      ...
      console.log('[emailService] Sent email via SendGrid API');
      return { ok: true, via: 'sendgrid' };
    } catch (err) {
      console.warn('[emailService] SendGrid failed, falling back to SMTP:', err && err.message ? err.message : err);
    }
  }
  ...
  console.log('[emailService] Sent via SMTP', info && info.messageId ? info.messageId : info);
