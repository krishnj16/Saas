...
function safeGetIo() {
  try {
    ...
    logger.warn('[socketEmitter] getIo() failed:', err.message);
    return null;
  }
}
...
function emitScanStarted(userId, { scan_task_id, website_id }) {
  ...
  else logger.info('[socketEmitter] emitScanStarted (no io) ->', payload);
}
...
function emitScanProgress(userId, { scan_task_id, percent, current_scanner }) {
  ...
  else logger.info('[socketEmitter] emitScanProgress (no io) ->', payload);
}
...
async function emitVulnFound(user, { scan_task_id, website_id, vuln }) {
  ...
  else logger.info('[socketEmitter] vuln:found (no io) ->', payload);
  ...
  if (severity === 'critical') {
    if (typeof emailService.sendCriticalVulnEmail === 'function') {
      try {
        ...
        logger.info('[socketEmitter] critical vuln email attempted for', user.email);
      } catch (err) {
        logger.error('[socketEmitter] Error sending critical vuln email:', err.message);
      }
    } else {
      logger.warn('[socketEmitter] emailService.sendCriticalVulnEmail not found - skipping email', { user: user.email, scan_task_id });
    }
  }
}
...
function emitScanFinished(userId, { scan_task_id, website_id, counts, summary }) {
  ...
  else logger.info('[socketEmitter] emitScanFinished (no io) ->', payload);
}
