...
let Worker;
try { Worker = require('bullmq').Worker; } catch (e) { process.exit(1); }

async function parseAndSaveFindings(websiteId, scanTaskId, jsonOutput) {
  try {
    if (!jsonOutput) return;

    if (Array.isArray(jsonOutput.interesting_findings)) {
      for (const item of jsonOutput.interesting_findings) {
        await pool.query(
          `INSERT INTO findings (website_id, scan_task_id, type, severity, description, status, created_at)
           VALUES ($1, $2, 'Info', 'Low', $3, 'Open', NOW())`,
          [websiteId, scanTaskId, item.to_s || item.url]
        );
      }
    }

    const vulns = jsonOutput.vulnerabilities || {};
    const allVulns = [
      ...(vulns.wordpress || []),
      ...(vulns.plugins || []).flatMap(p => p.vulnerabilities || []),
      ...(vulns.themes || []).flatMap(t => t.vulnerabilities || [])
    ];

    for (const v of allVulns) {
      await pool.query(
        `INSERT INTO findings (website_id, scan_task_id, type, severity, description, status, created_at)
         VALUES ($1, $2, 'Vulnerability', 'High', $3, 'Open', NOW())`,
        [websiteId, scanTaskId, v.title || 'Unknown Vulnerability']
      );
    }
    
    log(`[worker]  Parsed and saved ${allVulns.length} real vulnerabilities.`);
  } catch (e) {
    log(`[worker] Error parsing findings: ${e.message}`);
  }
}

async function processor(job) {
  const { websiteId, options } = job.data || {};
  if (!websiteId) throw new Error('Missing websiteId');

  const tRes = await pool.query(`INSERT INTO scan_tasks (website_id, status, created_at) VALUES ($1, 'queued', NOW()) RETURNING id`, [websiteId]);
  const scanTaskId = tRes.rows[0].id;

  const wRes = await pool.query('SELECT url FROM websites WHERE id = $1', [websiteId]);
  const websiteUrl = wRes.rows[0].url;

  await pool.query("UPDATE scan_tasks SET status = 'started', started_at = NOW() WHERE id = $1", [scanTaskId]);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scan-'));
  const image = 'wpscanteam/wpscan';
  const args = ['--url', websiteUrl, '--format', 'json', '--output', '/out/result.json', '--random-user-agent', '--disable-tls-checks'];
  
  const timeoutMs = 600000; 

  log(`[worker]  Starting REAL scan on ${websiteUrl} (this will take time)...`);
  
  const res = await runScannerDocker(image, args, timeoutMs, { mountHostDir: tmpDir });

  let scanOutputStr = "{}";
  let scanOutputJson = null;
  const resultFile = path.join(tmpDir, 'result.json');

  if (fs.existsSync(resultFile)) {
    scanOutputStr = fs.readFileSync(resultFile, 'utf8');
    try { scanOutputJson = JSON.parse(scanOutputStr); } catch(e) {}
  } else {
    log('[worker]  No result file found. Docker might have failed or permissions issue.');
  }

  await pool.query(
    `INSERT INTO scan_outputs (scan_task_id, scanner_name, raw_json, created_at) VALUES ($1, 'wpscan', $2, NOW())`,
    [scanTaskId, scanOutputJson || {}] 
  );

  if (scanOutputJson) {
    await parseAndSaveFindings(websiteId, scanTaskId, scanOutputJson);
  }

  try { fs.rmSync(tmpDir, { recursive: true, force: true }); } catch(e) {}

  await pool.query("UPDATE scan_tasks SET status = 'completed', finished_at = NOW() WHERE id = $1", [scanTaskId]);
...
async function startWorker() {
  ...
  worker.on('completed', (job) => log('[worker] Job completed', job.id));
  worker.on('failed', (job, err) => log('[worker] Job failed', job.id, err.message));
}
